...
import {
  DEFAULT_VALKEY_OPTIONS,
  ReconnectOnError,
  ValkeyOptions,
} from "./redis/ValkeyOptions.js";
...
/**
 * This is the major component of iovalkey.
 * Use it to connect to a standalone Valkey server or Sentinels.
 *
 * ```typescript
 * const valkey = new Valkey(); // Default port is 6379
 * async function main() {
 *   valkey.set("foo", "bar");
 *   valkey.get("foo", (err, result) => {
 *     // `result` should be "bar"
 *     console.log(err, result);
 *   });
 *   // Or use Promise
 *   const result = await valkey.get("foo");
 * }
 * ```
 */
// eslint-disable-next-line @typescript-eslint/no-unsafe-declaration-merging
class Valkey extends Commander implements DataHandledable {
  static Cluster = Cluster;
  static Command = Command;
  /**
   * Default options
   */
  private static defaultOptions = DEFAULT_VALKEY_OPTIONS;

  /**
   * Create a Valkey instance.
   * This is the same as `new Valkey()` but is included for compatibility with node-redis.
   */
  static createClient(...args: ConstructorParameters<typeof Valkey>): Valkey {
    return new Valkey(...args);
  }

  options: ValkeyOptions;
  status: ValkeyStatus = "wait";
  ...
  constructor(port: number, host: string, options: ValkeyOptions);
  constructor(path: string, options: ValkeyOptions);
  constructor(port: number, options: ValkeyOptions);
  ...
  /**
   * Create a connection to Valkey.
   * This method will be invoked automatically when creating a new Valkey instance
   ...
   * If you want to wait for the pending replies, use Valkey#quit instead.
   */
   ...
   * @example
   * ```js
   * var valkey = new Valkey(6380);
   * var anotherValkey = valkey.duplicate();
   * ```
   */
  duplicate(override?: Partial<ValkeyOptions>) {
    return new Valkey({ ...this.options, ...override });
  }
   ...
   *
   * This command will create a new connection to Valkey and send a
   ...
   * @example
   * ```js
   * var valkey = new Valkey();
   * valkey.monitor(function (err, monitor) {
   ...
   * valkey.monitor().then(function (monitor) {
   *   monitor.on('monitor', function (time, args, source, database) {
   *     console.log(time + ": " + util.inspect(args));
   *   });
   * });
   * ```
   */
  monitor(callback?: Callback<Valkey>): Promise<Valkey> {
  ...
  private parseOptions(...args: unknown[]) {
    ...
    defaults(options, Valkey.defaultOptions);
   ...
   */
  private setStatus(status: ValkeyStatus, arg?: unknown) {
    // @ts-expect-error
    if (debug.enabled) {
      debug(
        "status[%s]: %s -> %s",
  ...
  /**
   * Check whether Valkey has finished loading the persistent data and is able to
   * process commands.
   */
      ...
      if (!info.loading || info.loading === "0") {
        ...
        const retryTime =
          ...
          _this.options.maxLoadingRetryTime < loadingEtaMs
            ...
            : loadingEtaMs;
        debug(
          "Valkey server still loading, trying again in " + retryTime + "ms"
        );
        setTimeout(function () {
...
// eslint-disable-next-line @typescript-eslint/no-unsafe-declaration-merging
interface Valkey extends EventEmitter {
  ...
  on(event: ValkeyStatus, cb: () => void): this;
  once(event: ValkeyStatus, cb: () => void): this;
...
applyMixin(Valkey, EventEmitter);

addTransactionSupport(Valkey.prototype);
// eslint-disable-next-line @typescript-eslint/no-unsafe-declaration-merging, @typescript-eslint/no-empty-object-type
interface Valkey extends Transaction {}

export { Transaction, Valkey };
