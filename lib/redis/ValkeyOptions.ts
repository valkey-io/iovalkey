...
export interface CommonValkeyOptions extends CommanderOptions {
  ...
  /**
   * Whether or not to reconnect on certain Valkey errors.
   * This options by default is `null`, which means it should never reconnect on Valkey errors.
   * You can pass a function that accepts an Valkey error, and returns:
  ...
  readOnly?: boolean;
  /**
   * When enabled, numbers returned by Valkey will be converted to JavaScript strings instead of numbers.
  ...
  /**
   * This option is used internally when you call `valkey.monitor()` to tell Valkey
  ...
  /**
   *
   * By default, if the connection to Valkey server has not been established, commands are added to a queue
   * and are executed once the connection is "ready" (when `enableReadyCheck` is true, "ready" means
   * the Valkey server has loaded the database from disk, otherwise means the connection to the Valkey
  ...
  /**
   * When a Valkey instance is initialized, a connection to the server is immediately established. Set this to
   * true will delay the connection to the server until the first command is sent or `valkey.connect()` is called
   * explicitly.
   *
   * @default false
   */
...
export type ValkeyOptions = CommonValkeyOptions &
  SentinelConnectionOptions &
  StandaloneConnectionOptions;

export const DEFAULT_VALKEY_OPTIONS: ValkeyOptions = {
  // Connection
  port: 6379,
  host: "localhost",
  family: 4,
